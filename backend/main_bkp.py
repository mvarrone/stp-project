...
def get_data(device, retries=3):
    ...
    for attempt in range(retries):
        print(f"Attempt {attempt + 1}/{retries} for {host}:{port}")
        try:
...
def main():
    ...
    devices = load_credentials(CREDENTIALS_FILE)
    if not devices:
        sys.exit(1)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_device = {
            executor.submit(get_data, device): device for device in devices
        }
    ...
    if not successful_connections:
        # print("\nThere are no successful connections")
        return
